...
class WeChat(WeChatBase):
    def wexName(self):
        ...
        for hd in my_HWND:
            ...
            print(self.A_MyIcon.Name)
    def __init__(self, language='cn',type=1,hwnd=None) -> None:
        """微信UI自动化实例
        ...
        if type == 1:
            ...
            self.my_HWND_dict = ''
            # 枚举所有顶级窗口
            win32gui.EnumWindows(self.enum_window_proc, my_HWND)
            for hd in my_HWND:
                print(f"WeChatinf:my_HWND::{hd}")
                self.UiaAPI = uia.WindowControl(ClassName='WeChatMainWndForPC', searchDepth=1)
                self.HWND = hd;
                win32gui.ShowWindow(self.HWND, 1)
                win32gui.SetWindowPos(self.HWND, -1, 0, 0, 0, 0, 3)
                ...
                self.NavigationBox, self.SessionBox, self.ChatBox = MainControl2.GetChildren()
                self.A_MyIcon = self.NavigationBox.ButtonControl()
                print(f"WeChatinf:Name::{self.NavigationBox.ButtonControl().Name}")
                my_HWND_dict[self.NavigationBox.ButtonControl().Name] = hd
                self.my_HWND_dict = my_HWND_dict
                time.sleep(1)

    def enum_window_proc(self,hwnd, result_list):
        window_text = win32gui.GetClassName(hwnd)
        if "WeChatMainWndForPC" in window_text:
            print(f"WeChatMainWndForPC::hwnd")
    ...
    def GetListenMessage(self):
        ...
        try:
            ...
            print(f"获取监听对象的新消息失败:{e}")
        finally:
            print("释放锁")
            lock.release()


    def GetListenObj(self,who):
        """获取监听对象的新消息"""
        lock.acquire()
        try:
            chat = self.listen[who]

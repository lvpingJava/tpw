...
# 已实现社交聊天数字识别
# 模板
tempimg = cv2.imread('./numtest2.bmp')
...
cv2.drawContours(tempimg, contours, -1, (0, 0, 255), 2)
cv_show('ref',tempimg)
...
for (i, c) in enumerate(contours):
    ...
    roi = cv2.resize(roi,(15,20))
    digits[i] = roi  # 对应模板

print(f"digits大小：{len(digits)}")
# 测试
img = cv2.imread('./numtest1.bmp')
testimg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
testimg = cv2.threshold(testimg,  0, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)[1]

testcontours, testhierarchy = cv2.findContours(testimg.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
cv2.drawContours(img, testcontours, -1, (0, 0, 255), 1)
